...
public class UserCustomizationServiceImpl implements UserCustomizationService {
  ...
  public UserCustomizationResponse getUserCustomization(UUID userId) {
    ...
    List<String> backgroundUrls =
        backgrounds.stream().map(s3Service::getBackgroundUrl).collect(Collectors.toList());

    // 캐릭터 URL 목록 생성
    List<String> characterUrls =
        characters.stream().map(s3Service::getCharacterUrl).collect(Collectors.toList());
  ...
  public void addBackground(UUID userId, BackgroundRequest request) {
    ...
    UserBackground userBackground =
        UserBackground.builder().userId(userId).backgroundCode(backgroundCode).build();
  ...
  @Override
  @Transactional
  public CustomizationSelectResponse selectCustomization(
      UUID userId, CustomizationSelectRequest request) {
    log.debug(
        "커스터마이제이션 선택 요청. userId: {}, backgroundCode: {}, characterCode: {}",
        userId,
        request.getBackgroundCode(),
        request.getCharacterCode());
    ...
    if (request.getBackgroundCode() != null) {
      if (!userBackgroundRepository.existsByUserIdAndBackgroundCode(
          userId, request.getBackgroundCode())) {
    ...
    if (request.getCharacterCode() != null) {
      if (!userCharacterRepository.existsByUserIdAndCharacterCode(
          userId, request.getCharacterCode())) {
    ...
    log.info(
        "커스터마이제이션 선택 완료. userId: {}, backgroundCode: {}, characterCode: {}",
        userId,
        user.getProfile().getCurrentBackgroundCode(),
        user.getProfile().getCurrentCharacterCode());
