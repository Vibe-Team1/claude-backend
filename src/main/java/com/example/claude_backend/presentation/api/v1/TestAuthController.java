...
/**
 * 개발/테스트용 인증 컨트롤러 OAuth2 없이 직접 토큰을 발급받을 수 있음
 *
 * @author AI Assistant
 * @since 2025-01-20
 */
...
public class TestAuthController {
  ...
  public ResponseEntity<ApiResponse<Map<String, String>>> testLogin(
    ...
    try {
      // 이메일로 사용자 조회 또는 생성
      User user =
          userService.processOAuthLogin(
              email, "test_google_sub_" + email.hashCode(), email.split("@")[0], null);
      ...
      return ResponseEntity.badRequest().body(ApiResponse.error("LOGIN_FAILED", e.getMessage()));
    }
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> verifyToken(
    ...
    if (jwtTokenProvider.validateToken(token)) {
      ...
      return ResponseEntity.badRequest().body(ApiResponse.error("INVALID_TOKEN", "유효하지 않은 토큰입니다."));
    }
  }
}
