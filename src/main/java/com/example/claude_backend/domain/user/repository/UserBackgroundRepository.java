...
public interface UserBackgroundRepository extends JpaRepository<UserBackground, Long> {
    ...
    @Query("SELECT ub FROM UserBackground ub WHERE ub.userId = :userId AND ub.backgroundCode = :backgroundCode")
    Optional<UserBackground> findByUserIdAndBackgroundCode(@Param("userId") UUID userId,
            @Param("backgroundCode") String backgroundCode);

    @Query("SELECT COUNT(ub) > 0 FROM UserBackground ub WHERE ub.userId = :userId AND ub.backgroundCode = :backgroundCode")
    boolean existsByUserIdAndBackgroundCode(@Param("userId") UUID userId,
            @Param("backgroundCode") String backgroundCode);
}